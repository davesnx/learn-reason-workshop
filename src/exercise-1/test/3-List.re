...
describe("List", ({test}) => {
  test("Basic", ({expect}) => {
    ...
    let rec sum = lst => List.length(lst);
     ...
     */
    let addHead = (hd, rest) => [hd, ...rest];

    expect.int(sum([])).toBe(0);
    expect.int(sum([55])).toBe(55);
    expect.int(sum([5, (-5), 1, (-1)])).toBe(0);
    expect.int(sum([5, 5, 1, 1])).toBe(12);
  });

  test("Operations", ({expect}) => {
    /*
       ...
       take other functions as input. Lets take a look at the [List.map] function,
       which has signature:

         let map : (('a => 'b), list('a)) => list('b)

       Lets read this signature together. It takes two arguments:
     ...
     */
    let () =
      List.iter(i => Js.log("here's an int: " ++ i), myStrings(myInts));
    ...
    /*
       ...
       Lets say your list [l] contains [a1, a2, a3]. Then if you call
    ...
    expect.list(myNewInts(myInts)).toEqual([2, 3, 4, 5, 6]);
    expect.int(numEvenInts(myInts)).toBe(2);
  });
});
